...
```dart
// text-wrap: balance equivalent — constrain and allow soft wrap with overflow guard
Text(
  heading,
  softWrap: true,
...
// font-variant-numeric: tabular-nums equivalent
Text(
  price,
  style: TextStyle(
...
```
<!-- Flutter-ized today: tabular-nums and text-wrap balance equivalents added -->
...
```
<!-- Flutter-ized today: macOS/web-only note added -->

> **Flutter note:** `-webkit-font-smoothing` is macOS/web only — no Flutter equivalent, skip.
...
```dart
// img outline equivalent — inset border via BoxDecoration
Container(
  decoration: BoxDecoration(
    border: Border.all(
      color: Colors.black.withOpacity(0.1), // use Colors.white.withOpacity(0.1) for dark mode
      width: 1,
    ),
  ),
...
```
<!-- Flutter-ized today: img outline via Container BoxDecoration border added -->
...
```dart
// CSS transitions equivalent — use AnimatedContainer, AnimatedOpacity, or TweenAnimationBuilder
// with AppDurations tokens instead of hardcoded durations

// AnimatedContainer for color/size/decoration changes:
AnimatedContainer(
  duration: AppDurations.short, // e.g. 150ms
  curve: Curves.easeOut,
  decoration: BoxDecoration(color: isPressed ? pressedColor : defaultColor),
  child: child,
)

// AnimatedOpacity for enter/exit:
AnimatedOpacity(
...
// TweenAnimationBuilder for scale (tactile press, icon swap):
TweenAnimationBuilder<double>(
  tween: Tween(begin: 1.0, end: isPressed ? 0.96 : 1.0),
  duration: AppDurations.short,
  curve: Curves.easeOut,
...
```
<!-- Flutter-ized today: CSS transitions mapped to AnimatedContainer/AnimatedOpacity/TweenAnimationBuilder with AppDurations -->
...
```dart
// Hit area expansion — wrap small tap targets to >=48x48 logical px
SizedBox(
  width: 48,
  height: 48,
  child: Padding(
    ...
    child: GestureDetector(
      onTap: onTap,
      child: icon, // visually small, but touch target is 48x48
    ),
  ),
)
```
<!-- Flutter-ized today: hit area expansion via SizedBox+Padding+GestureDetector added -->
