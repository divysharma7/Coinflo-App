...
## 8. Drift DAO + Riverpod StreamProvider

```dart
// 1. Drift DAO — exposes a select().watch() stream
// data/daos/transactions_dao.dart
part of '../db.dart';
...
class TransactionsDao extends DatabaseAccessor<AppDatabase>
  ...
  /// Returns a live stream of all transactions ordered by date descending.
  Stream<List<Transaction>> watchAll() =>
      (select(transactions)..orderBy([(t) => OrderingTerm.desc(t.date)]))
          .watch();

  /// Returns a live stream of transactions belonging to a single account.
  Stream<List<Transaction>> watchByAccount(int accountId) => (select(transactions)
        ..where((t) => t.accountId.equals(accountId))
        ..orderBy([(t) => OrderingTerm.desc(t.date)]))
      .watch();
}

// 2. Riverpod @riverpod stream provider wrapping the DAO stream
// providers/transactions_providers.dart
import 'package:riverpod_annotation/riverpod_annotation.dart';

part 'transactions_providers.g.dart';

/// Exposes all transactions as a reactive stream.
/// Auto-disposed; re-subscribes automatically when the widget tree needs it.
@riverpod
...
/// Filtered stream for a specific account.
@riverpod
Stream<List<Transaction>> transactionsByAccount(Ref ref, int accountId) {
  final dao = ref.watch(appDatabaseProvider).transactionsDao;
  return dao.watchByAccount(accountId);
}

// 3. ConsumerWidget consuming AsyncValue from the stream provider
// pages/transactions/transactions_page.dart
class TransactionsPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return switch (asyncTransactions) {
      AsyncData(:final value) => _TransactionList(transactions: value),
...
<!-- Flutter-ized today: replaced Dio HTTP client + Crashlytics error sections with Drift DAO select().watch() -> Stream -> @riverpod StreamProvider -> AsyncValue ConsumerWidget pattern -->
